...
/**
 * bitonic_merge_down - Merges two subarrays in descending order
 * @array: Pointer to the array
 * @low: Starting index of the first subarray
 * @count: Number of elements to be merged
 */
void bitonic_merge_down(int *array, size_t low, size_t count)
{
	if (count > 1)
	{
		size_t k = count / 2;
		...
		bitonic_merge_down(array, low, k);
		bitonic_merge_down(array, low + k, k);
	}
}

/**
 * bitonic_sort_recursive_up - func for bitonic sort in ascending order
...
void bitonic_sort_recursive_up(int *array, size_t low, size_t count)
{
	if (count > 1)
	{
		...
		bitonic_merge_down(array, low, count);
...
/**
 * bitonic_sort - Sorts array of ints in ascending order using Bitonic sort
 * @array: Pointer to the array to be sorted
